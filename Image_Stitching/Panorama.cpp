...
BYTE* PanoToResized(BYTE* Raw, int Width, int Height, xy newPanoSize, xy position) {
	...
	int i, j;
	int tempR = 0, tempG = 0, tempB = 0, bufpos, bufpos2;
	BYTE* yeni = new BYTE[newPanoSize.x * newPanoSize.y * 3];
	/*BYTE* yeni = new BYTE[newPanoSize.x * newPanoSize.y * 3]();*/
	...
	for (i = 0; i < newPanoSize.y; i++) {
		for (j = 0; j < newPanoSize.x; j++) {
			bufpos = (newPanoSize.y - i - 1) * newPanoSize.x * 3 + j * 3;
	...
	for (i = 0; i < Height; i++) {
		for (j = 0; j < Width; j++) {
			buf1 = (newPanoSize.y - (position.y + i) - 1) * newPanoSize.x * 3 + (j + position.x) * 3;
...
BYTE* BoyutAyarla(double** H, BYTE* Raw, int Width, int Height, xy Size, xy position, int img) {
	// img  etiketi ile gelecek 1 veya 2 nolu imgeleri, Birleþmesi ile oluþacak Size.x * Size.y  boyutundaki imgeye : 
	//  1. önce tüm piksellerine görüntünün ortalama RGB deðerlerini yerleþtir,
	//  2. Raw deðiþkeni ile gelen imgeyi Size.x * Size.y  boyutundaki imgedeki konumuna konuþlandýrarak kopyala 
	int i, j;
	int tempR = 0, tempG = 0, tempB = 0, bufpos;
	...
	if (img == 1) {
		...
		for (i = 0; i < Height; i++) {
			for (j = 0; j < Width; j++) {
				yeniPos = (Size.y - (position.y + i) - 1) * Size.x * 3 + (j + position.x) * 3;
		...
		for (i = 0; i < Height; i++) {
			for (j = 0; j < Width; j++) {
				point = Transfer(H, j, i);
				yeniPos = (Size.y - (position.y + point.y) - 1) * Size.x * 3 + (point.x + position.x) * 3;
				rawPos = (Height - i - 1) * Width * 3 + j * 3;
				if (yeniPos > 0) {
					yeni[yeniPos] = Raw[rawPos];
					yeni[yeniPos + 1] = Raw[rawPos + 1];
